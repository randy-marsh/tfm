...
def fog_hour_variance(mat) -> numpy.array:
    """
    Obtain the std of the minimum rvr per hour
    :param mat: mat file descriptor
    :return pandas.DataFrame vector: DataFrame with fog duration, min RVR values
    """
    foghourfreq = list()
    fogrvrmin = list()
    ...
    df = pandas.DataFrame(numpy.column_stack((foghourfreq, fogrvrmin)))
    return df.groupby(0).std()
...
def main(input_path: str, output_path: str):
    ...
    fog_variance = fog_hour_variance(mat)
    print(fog_variance)
